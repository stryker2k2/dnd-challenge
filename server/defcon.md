# DEFCON 34
*** Welcome to DEFCON 34! ***
Disclaimer: Connecting with PuTTY is NOT supported!
You walk into the DEFCON 34 Village and notice a sea 
of hackers sitting around tables with their laptops. 
You notice one table by itself with a lone laptop on 
display with a text-based game running.
What do you do?

[1] Play the game
    The game is a Dungeons and Dragons type text-based game.
    It wants you to pick a name. "Champion, what name do you
    hail by?"

    [>] Name Entry

        Hail, %s! You find yourself at a Tavern
        ordering yourself a drink. What do you order?

        [1] Ale, of course!

            The Bartender slides a cold frosty mug of the finest
            ale this side of Buldur's Bridge. He fills a mug up for
            himself and raises it high. He suggests that you propose
            a toast. To what cause shall we drink to?

            [>] Toast Entry

                You, the mighty %s, stand up proud with your frosty
                mug of ale and make a toast to %s!
                The entire tavern erupts into loud bolsterous cheer
                and celebration! Cheers to %s!

                [>] Return to Main Menu or Disconnect

        [2] Bartender's Choice

            The Bartender scratches his head out of disbelief. After all,
            a mighty Warrior with a frightening name like %s
            should obviously know what they want to drink.

            [>] Return to Main Menu or Disconnect

        [3] Water, please.

            The Bartender grabs a pitcher full of water and pours it
            over your head; soaking your garments and causing the
            entire Tavern to roar with laughter.

            [>] Return to Main Menu or Disconnect

        [4] Return to Main Menu
        [5] Terminate Connection

[2] Search the office (Roll a 3-sided dice for Perception)

    [1] You roll the three-sided dice and the number 1 is facing
        up. Your critical lack of perception focuses on the
        first thing you see - a ping pong table.
        What shall you do now?

        [1] Grab a paddle and flip it upwards in the air

            Smoother than an Olympic Diver and as cunning as the
            local Brazilian Jiu-Jitsu student, you gracefully fling the
            paddle upwards as it does a 720 degree backwards flip
            and land elegantly back in your hand.

            [>] Return to Main Menu or Disconnect

        [2] Try to bounce the ping pong ball on the paddle

            You firmly grab the ping pong paddle and with fierce
            determination, you drop the ping pong ball onto the
            paddle and start bouncing it on the paddle. Ping - pong - 
            ping - pong - you successfully bounce the ball
            times before it bounces out of your control!

            [>] Return to Main Menu or Disconnect

        [3] Return to Main Menu
        [4] Terminate Connection

    [2] You roll the three-sided dice and the number 2 is
        facing up. Your average amount of perception notices a
        fancy coffee machine in the break room.
        What shall you do now?

            [1] Eagerly brew yourself a cup of coffee!

                You eagerly make your amazing coffee selection and wait
                forever for it to brew. It is too hot to drink right now but
                those freshly ground beans make it worth the wait!

                [>] Return to Main Menu or Disconnect

            [2] Kindly brew a cup for the Reverse Engineer guy

                You calmly walk over to the resident Reverse Engineer guy and
                give him the cup of freshly brewed coffee. He smiles, takes
                a sip, then starts to think outloud to himself. He says,
                "Ya know, I think there is a bug in my program. A user could
                easily overflow the buffer. Hmmm, I should fix that."

            [3] Return to Main Menu
            [4] Terminate Connection

    [3] You roll the three-sided dice and the number 3 is
        facing up. Your exceptional perception notices a Rival
        Nemesis MXVII-10K NERF GUN on the conference table!
        What shall you do now?

            [1] Grab the Nerf Gun and go hunting for the BLS Owners

                You quickly grab the Nerf Nemesis Machine Gun with both
                hands and sprint into the nearest Owner's office screaming
                "MEET MY LITTLE FRIEND\" at the top of your lungs. But,
                instead of a surprise attack - you walk into an ambush. All the
                Owners unleash Nerf Fury upon you as you walk into the door.

                [>] Return to Main Menu or Disconnect

            [2] Set up a Nerf Land Mine near the Nerf Machine Gun

                With amazing skill and finesse, you set up the Nerf Land 
                Mine positioned perfect near the Nerf Nemesis Machine 
                Gun and lie patiently in the other room with the remote 
                detonator. One of the Owners walk towards it with delight 
                in their eyes! As you press the Remote Detonator, you 
                quickly remember that \"This Side Towards Enemy\" was 
                facing you! You get pelted by your own Nerf Land Mine!

                [>] Return to Main Menu or Disconnect

            [3] Return to Main Menu
            [4] Terminate Connection

[3] Hack Black Lantern Security

    Interesting Move. How would you do it?

    [>] Hack Input

        [1] You attempt that but the network is too segmented!

        [2] That method doesn't work here without the use of 
            the "--do-forcefully" flag. Rerun and try again.
        
        [3] Segmentation fault (core dumped). Better luck next time.

        [4] Stack Trace: undefined result
            at bls-party (function: idontwannacry.cs:14)
            at bls-party (function: youmadcowbro.cs:45)
            at bls-party (function: styleguide-for-400.nist:800-63)

        [5] Username is not in the sudoers file. This incident will be reported.

        [>] Return to Main Menu or Disconnect

[4] Turn off the Christmas Music

    You're a mean one, Mr. Grinch!

    [>] Return to Main Menu or Disconnect

[5] Terminate Connection